...
export default function LoginScreen() {
  const { login, kickedOutMessage, clearKickedOutMessage } = useAuth();
  ...
  const handleLogin = async () => {
    ...
    try {
      ...
      if (typeof e?.remainingAttempts === "number") {
        setError(e.remainingAttempts > 0
          ? t("login.error_invalid_creds", { count: e.remainingAttempts })
          : t("login.error_login"));
      } else if (typeof e?.minutesLeft === "number") {
        setError(t("login.error_account_locked", { minutes: e.minutesLeft }));
      } else if (e?.vpnDetected) {
        setError(t("login.error_vpn_detected"));
      } else if (e?.accountSuspended) {
        setError(t("login.error_suspended"));
      } else {
        setError(e?.message ?? t("login.error_login"));
      }
    } finally {
      setLoading(false);
    }
      ...
      <KeyboardAvoidingView style={{ flex: 1 }} behavior={Platform.OS === "ios" ? "padding" : "height"}>
        ...
        >
          ...
          <View style={styles.card}>
            <Text style={styles.cardTitle}>{t("login.card_title")}</Text>
            <Text style={styles.cardSub}>{t("login.card_sub")}</Text>

            {!!kickedOutMessage && (
              <View style={[styles.errorBox, { backgroundColor: "#FFFBEB", borderColor: "#FCD34D" }]}>
                <Feather name="alert-octagon" size={14} color="#B45309" />
                <Text style={[styles.errorText, { color: "#92400E" }]}>{kickedOutMessage}</Text>
              </View>
            )}
            ...
            >
              <LinearGradient colors={["#1A3561", "#0D1F3C"]} start={{ x: 0, y: 0 }} end={{ x: 1, y: 1 }} style={styles.btnGrad}>
                {loading ? (
                  ...
                  <>
                    <Text style={styles.btnText}>{t("login.btn_login")}</Text>
            ...
            <Link href="/(auth)/register" asChild>
              <TouchableOpacity style={styles.registerBtn} activeOpacity={0.8}>
                <Text style={styles.registerBtnText}>{t("login.btn_register")}</Text>
              </TouchableOpacity>
            </Link>
          </View>
          ...
          <View style={styles.footer}>
            <View style={styles.footerBadge}>
              ...
              <Text style={styles.footerText}>{t("login.footer_badge")}</Text>
            </View>
          </View>
        </ScrollView>
