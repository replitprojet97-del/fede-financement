...
function StarRow({ note, size = 14 }: { note: number; size?: number }) {
  return (
    <span className="flex items-center gap-0.5">
      {[1, 2, 3, 4, 5].map(i => (
        <Star key={i} className={`fill-current ${i <= note ? "text-[#FFD500]" : "text-gray-200"}`} style={{ width: size, height: size }} />
      ))}
    </span>
  );
}
        ...
        <div className="bg-white border border-[#DDE2EC] rounded-2xl p-6 mb-6 shadow-sm">
              ...
              <div className="flex items-center gap-2">
                {[1,2,3,4,5].map(n => (
                  <button key={n} onClick={() => setForm(f => ({ ...f, note: n }))}
                    className={`w-8 h-8 rounded-lg text-sm font-bold transition-colors ${form.note >= n ? "bg-[#FFD500] text-white" : "bg-gray-100 text-gray-400"}`}>
                    {n}
                  </button>
                ))}
              </div>
          ...
          <div className="flex gap-3">
            <button onClick={addReview} disabled={submitting}
              className="bg-[#FFD500] text-white px-5 py-2 rounded-lg text-sm font-semibold hover:bg-[#FFC900] transition-colors disabled:opacity-50">
