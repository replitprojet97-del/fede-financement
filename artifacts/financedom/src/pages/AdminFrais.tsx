...
import { Mail, CheckCircle, Building2, AlertTriangle } from "lucide-react";
...
export default function AdminFrais() {
  ...
  useEffect(() => {
    ...
    fetch(`${BASE}/api/admin/coordonnees-bancaires`, { credentials: "include" })
      ...
      .then(d => setCoordonnees(d))
      .catch(() => setCoordonnees(null))
      .finally(() => setCoordLoading(false));
  }, []);

  const eligibleDossiers = dossiers.filter(d => d.statut !== "brouillon" && d.statut !== "rejete");
  ...
  const hasBankDetails = !!(coordonnees?.iban && coordonnees?.beneficiaire);
      ...
      <AdminLayout>
        <div className="flex items-center justify-center min-h-[60vh]">
          <div className="text-center bg-white border border-gray-200 rounded-xl shadow-sm p-10 max-w-md w-full">
            ...
            <p className="text-gray-500 text-sm mb-2">L'avis de frais a été envoyé au porteur du dossier par e-mail et dans la messagerie.</p>
            <p className="text-gray-400 text-xs mb-8">Il peut désormais procéder au virement bancaire depuis son espace personnel.</p>
            <button
        ...
        <div className="md:col-span-8 space-y-5">
          ...
          <div className="bg-white border border-gray-200 rounded-xl shadow-sm p-6">
            <div className="flex items-center gap-2 mb-4">
              <Building2 className="w-4 h-4 text-[#0f1f3d]" />
              <h3 className="font-bold text-[#0f1f3d] text-base">3. Coordonnées bancaires de réception</h3>
            </div>
            <p className="text-gray-400 text-xs mb-4">Ces coordonnées seront incluses dans l'e-mail et le message interne envoyés au porteur pour lui permettre d'effectuer le virement.</p>

            {coordLoading ? (
              <div className="h-20 flex items-center justify-center text-gray-400 text-sm">Chargement…</div>
            ) : !hasBankDetails ? (
              <div className="flex items-start gap-3 bg-amber-50 border border-amber-200 rounded-xl p-4">
                <AlertTriangle className="w-4 h-4 text-amber-600 mt-0.5 shrink-0" />
                <div>
                  <p className="text-amber-700 font-semibold text-sm">Coordonnées bancaires non configurées</p>
                  <p className="text-amber-600/80 text-xs mt-1">
                    Rendez-vous dans <strong>Paramètres → Coordonnées bancaires</strong> pour saisir l'IBAN et les informations de virement avant d'émettre un avis de frais.
                  </p>
                </div>
              </div>
            ) : (
              <div className="bg-green-50 border border-green-200 rounded-xl p-4 space-y-2">
                <div className="grid grid-cols-2 gap-x-4 gap-y-2 text-sm">
                  <div>
                    <div className="text-xs text-gray-500 font-semibold uppercase tracking-wide mb-0.5">Bénéficiaire</div>
                    <div className="text-[#0f1f3d] font-bold">{coordonnees!.beneficiaire}</div>
                  </div>
                  <div>
                    <div className="text-xs text-gray-500 font-semibold uppercase tracking-wide mb-0.5">Banque</div>
                    <div className="text-[#0f1f3d]">{coordonnees!.banque}</div>
                  </div>
                  <div className="col-span-2">
                    <div className="text-xs text-gray-500 font-semibold uppercase tracking-wide mb-0.5">IBAN</div>
                    <div className="text-[#0f1f3d] font-mono font-bold tracking-wider">{coordonnees!.iban}</div>
                  </div>
                  <div>
                    <div className="text-xs text-gray-500 font-semibold uppercase tracking-wide mb-0.5">BIC / SWIFT</div>
                    <div className="text-[#0f1f3d] font-mono">{coordonnees!.bic}</div>
                  </div>
                  <div>
                    <div className="text-xs text-gray-500 font-semibold uppercase tracking-wide mb-0.5">Domiciliation</div>
                    <div className="text-[#0f1f3d]">{coordonnees!.domiciliation}</div>
                  </div>
                  <div className="col-span-2">
                    <div className="text-xs text-gray-500 font-semibold uppercase tracking-wide mb-0.5">Libellé de virement</div>
                    <div className="text-blue-700 font-mono text-xs bg-blue-50 px-2 py-1 rounded">{coordonnees!.libelleVirement}</div>
                  </div>
                </div>
                <p className="text-xs text-green-700 mt-2">✓ Ces coordonnées seront transmises au porteur dans l'e-mail et la messagerie.</p>
              </div>
            )}
          </div>

          {/* STEP 4 — Confirmer et envoyer */}
          <div className="bg-white border border-gray-200 rounded-xl shadow-sm p-6">
            <h3 className="font-bold text-[#0f1f3d] text-base mb-4">4. Confirmer et envoyer</h3>
            <div className="bg-amber-50 border border-amber-200 rounded-xl p-4 mb-5 text-sm">
              ...
              <p className="text-amber-700/80 text-xs leading-relaxed">
                L'avis de frais sera envoyé par e-mail et dans la messagerie à {selectedDossier ? <strong>{selectedDossier.user?.prenom} {selectedDossier.user?.nom}</strong> : "l'utilisateur sélectionné"}, avec les coordonnées bancaires pour le virement.
              </p>
            </div>
            <button
              onClick={handleEmit}
