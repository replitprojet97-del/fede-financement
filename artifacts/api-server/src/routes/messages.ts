...
router.get("/dossiers/:id/messages", requireAuth, async (req, res): Promise<void> => {
  ...
  // Marque "lu" uniquement les messages destinés au porteur (admin/system → user).
  // Ne touche pas aux messages que le porteur a lui-même envoyés à l'admin :
  // ceux-ci doivent rester non-lus tant que l'admin ne les a pas ouverts.
  await db
